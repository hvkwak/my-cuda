...
int main(int argc, char **argv)
{
    ...
    // exercise 3-1: What are the two primary causes of performance improvement
    // when unrolling loops, data blocks, or warps in CUDA? Explain how each
    // type of unrolling improves instruction throughput.
    //
    // the two primary causes of performance improvement are that:
    // 1. it reduces instruction overheads, and
    // 2. it creates more independent instructions, e.g. independent load and store
    //    that more concurrent operations are added to the pipeline leading to higher
    //    saturation of instruction and memory bandwidth. Warp scheduler has more
    //    eligible warps that can help hide instruction or memory latency.
